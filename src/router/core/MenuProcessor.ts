...
export class MenuProcessor {
  ...
  private async processBackendMenu(): Promise<AppRouteRecord[]> {
    console.log('[MenuProcessor] 开始获取后端菜单...')
    const list = await fetchGetMenuList()
    console.log('[MenuProcessor] 后端返回菜单数据:', JSON.stringify(list, null, 2))
    const filtered = this.filterEmptyMenus(list)
    console.log('[MenuProcessor] 过滤后菜单数据:', JSON.stringify(filtered, null, 2))
    return filtered
  }
